...
Helm charts that deploy a complete Platform Engineering workshop environment on OpenShift 4.x. Designed for use with the [etx-app-platform-collection](https://github.com/evanshortiss/etx-app-platform-collection) Ansible collection, which bootstraps the cluster and deploys a single Argo CD `Application` CR pointing to the `app-of-apps/` chart. From there, Argo CD manages everything.
